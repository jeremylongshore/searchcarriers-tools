...
description: "Analyzes verified SearchCarriers evidence for carrier relationship mapper. Use when a user asks, \"Map companies connected to DOT 1234567 through\u2026\". Trigger with \"Map companies connected to DOT\u2026\"."
...
Carrier Relationship Mapper helps an operator map evidence of shared equipment and related company records without declaring common control. It solves this operational failure: Fraud investigations often overstate weak shared attributes, and common addresses or service providers create noisy links.
...
The bounded result is: Return CONFIRMED LINK only for an exact shared identifier, POSSIBLE LINK for weaker API evidence, and UNRESOLVED when evidence is incomplete.
...
**Routes/tools:** GET /api/v3/company/{dot}; GET /api/v3/company/{dot}/equipment; GET /api/v1/search/by-vin/{vin}

Start from a confirmed DOT, collect equipment VINs, resolve each VIN, exclude the seed entity, and retain the exact shared identifier for every edge.
...
Capture: Seed identity, related DOT, shared VIN or API-provided association, dates where present, source route, and confidence rationale.
...
**Next action:** Manually verify high-impact links through official records and trusted contacts before changing carrier status.
...
**Should trigger:** “Map companies connected to DOT 1234567 through shared VINs.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Decode one VIN for its make and model.”
...
| Missing field or empty data | No related results is not proof of independence; state the coverage limits and unsearched identifiers. |
