...
description: "Analyzes carrier evidence for auditable carrier exporter. Use when a user asks, \"Export our approved carrier panel to formula-safe\u2026\". Trigger with \"Export our approved carrier panel\u2026\"."
...
Auditable Carrier Exporter helps an operator create a minimal, traceable carrier export that is safe to open and lawful to retain. It solves this operational failure: Exports lose field provenance, leak unnecessary contacts, or execute spreadsheet formulas when opened.
...
The bounded result is: Return READY, PARTIAL, or BLOCKED with output path and manifest; never imply a partial export is complete.
...
**Routes/tools:** GET /api/v1/export; GET /api/v3/company/{dot} with explicit fields

Define purpose and minimum fields, fetch under plan limits, preserve source/as-of and policy result, escape CSV formula prefixes, and write a manifest with record/error counts.
...
Capture: Selected fields, schema version, source routes, as-of, input hash, record count, errors, and retention/recipient note.
...
**Next action:** Deliver through an approved internal channel and delete temporary API data under the organization retention policy.
...
**Should trigger:** “Export our approved carrier panel to formula-safe CSV with provenance.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Check authority for one carrier.”
...
| Missing field or empty data | Missing fields remain blank with a reason. 403 stops unsupported data; 429 resumes after Retry-After. |
