...
description: "Analyzes carrier evidence for trusted contact consistency check. Use when a user asks, \"The rate confirmation came from a new email;\u2026\". Trigger with \"The rate confirmation came from a\u2026\"."
...
Trusted Contact Consistency Check helps an operator compare supplied contact details with carrier records and define an independent callback step. It solves this operational failure: Spoofed email, caller ID, and recently changed FMCSA contacts make a matching string insufficient proof of identity.
...
The bounded result is: Return CONSISTENT, MISMATCH, or UNVERIFIED. Never claim that the API alone verified the person communicating.
...
**Routes/tools:** GET /api/v3/company/{dot}?fields=contact,risk_factors; GET /api/v1/company/{dot}/contact-details

Normalize but retain originals, compare phone/email/address field by field, inspect returned change or risk evidence, and verify material changes through a previously trusted channel.
...
Capture: Supplied contact, returned contact, comparison result, freshness/change evidence, verification channel, missing fields, and as-of.
...
**Next action:** Call a known number, confirm dispatch and equipment details, and hold the load until a material mismatch is resolved.
...
**Should trigger:** “The rate confirmation came from a new email; compare it with DOT 1234567.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Generate a fleet PDF.”
...
| Missing field or empty data | Missing contacts are UNVERIFIED. Generic or VoIP metadata may raise review but is not proof of fraud. |
