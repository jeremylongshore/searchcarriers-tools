...
description: "Analyzes carrier evidence for resumable carrier batch runner. Use when a user asks, \"Refresh our 500-carrier panel and make the run\u2026\". Trigger with \"Refresh our 500-carrier panel and\u2026\"."
...
Resumable Carrier Batch Runner helps an operator process a carrier panel with bounded concurrency, per-record receipts, and safe restart. It solves this operational failure: Large panels fail partially because of rate limits, bad identifiers, or tier errors; rerunning everything wastes quota and hides which records changed.
...
The bounded result is: Return COMPLETE, PARTIAL, or FAILED with a machine-readable error manifest; never discard successful rows because one carrier failed.
...
**Routes/tools:** GET /api/v1/export; GET /api/v3/company/{dot}; GET /api/v2/company/{dot}/qualification-reports

Validate and deduplicate input, assign a run ID, process bounded pages/batches, honor Retry-After, checkpoint after each carrier, and retry only transient failures.
...
Capture: Input hash, run ID, per-DOT status, response source/as-of, attempts, error class, checkpoint, and final success/review/failure counts.
...
**Next action:** Resume from the checkpoint, then route completed evidence into qualification or export.
...
**Should trigger:** “Refresh our 500-carrier panel and make the run resumable.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Explain one carrier safety rating.”
...
| Missing field or empty data | Unknown DOTs and permanent 4xx errors stay in the manifest. A 429 or 5xx is retryable within a bounded budget. |
