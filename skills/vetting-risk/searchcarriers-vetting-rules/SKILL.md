...
description: "Analyzes carrier evidence for policy qualification runner. Use when a user asks, \"Run our refrigerated-customer qualification for DOT\u2026\". Trigger with \"Run our refrigerated-customer\u2026\"."
...
Policy Qualification Runner helps an operator apply named company or customer criteria and return reproducible Pass, Review, or Fail evidence. It solves this operational failure: Informal checklists drift between analysts, while hidden defaults create decisions no one can audit.
...
The bounded result is: Return the upstream or deterministic policy verdict. Missing evidence must follow the rule policy and may never silently pass.
...
**Routes/tools:** GET /api/v2/company/{dot}/qualification-reports; GET /api/v3/company/{dot}?fields=vetting_report,risk_factors,authorities,insurance,safety,operation

Select the named qualification, preserve its version or criteria snapshot, use the upstream qualification result when available, and evaluate only caller-approved supplemental rules.
...
Capture: Qualification name, criteria/as-of, per-rule observed value and source, Pass/Review/Fail result, missing evidence, and override record.
...
**Next action:** Send failures to remediation, reviews to a human queue, and store the evidence snapshot with the decision.
...
**Should trigger:** “Run our refrigerated-customer qualification for DOT 1234567.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Is this phone number formatted correctly?”
...
| Missing field or empty data | If no named policy is supplied, ask for one or return evidence-only REVIEW; do not invent industry-standard thresholds. |
