...
description: "Analyzes verified SearchCarriers evidence for compliance snapshot builder. Use when a user asks, \"Build a current compliance snapshot and tell me\u2026\". Trigger with \"Build a current compliance\u2026\"."
...
Compliance Snapshot Builder helps an operator produce a point-in-time carrier compliance snapshot and identify what must be rechecked. It solves this operational failure: One-time onboarding checks age immediately, and teams often mistake a current snapshot for continuous monitoring.
...
The bounded result is: Return CURRENTLY CLEAR, ACTION REQUIRED, or REVIEW REQUIRED; separately state whether monitoring is configured.
...
**Routes/tools:** GET /api/v3/company/{dot}?fields=authorities,insurance,safety,oos_orders,operation,risk_factors; GET /api/v1/company/{dot}/watch

Fetch current evidence, label its as-of time, compare it only to explicit policy, and manage watch types when the user asks for future change coverage.
...
Capture: DOT status, authorities, insurance filings, safety/OOS evidence, watch state, missing data, policy source, and as-of timestamp.
...
**Next action:** Set the appropriate watch types and define an external review cadence or notification input.
...
**Should trigger:** “Build a current compliance snapshot and tell me what is not continuously monitored.”

Produce the bounded record, show the decisive evidence and unknowns, and give one
operational next action.

**Should not trigger:** “Export these records to CSV.”
...
| Missing field or empty data | API or field absence creates REVIEW. Watch configuration without an alert feed is not proof that delivery is working. |
